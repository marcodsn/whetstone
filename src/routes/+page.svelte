<script lang="ts">
    ...
    function doExport() {
        const blob = new Blob([exportData([...selected])], {
    ...
    function doReset() {
        if (
            confirm(
                "Erase all attempt history? This cannot be undone (consider exporting first).",
            )
        ) {
            clearAttempts();
...
<div class="page">
    <header class="post-header">
        <h1>Whetstone</h1>
        <p class="post-subtitle">To keep your mind sharp.</p>
        <div class="resource-links">
            <a class="resource-link primary" href="/session?mode=daily&n=10"
                >Begin today's session</a
            >
    ...
    <div class="article-body">
        <p class="intro-text">
            Large Language Models are doing all the thinking. They write our
            code, draft our emails, and translate our text. While productivity
            may be skyrocketing, our brains are getting too comfortable. This is
            why I created Whetstone, to make our brains struggle again — while
            also learning new stuff!
        </p>
        <p class="intro-text">
            {exercises.length} exercises across {DOMAINS.length} domains, with an
            Elo-style rating per domain so you can keep challenging yourself.
            {#if totalAttempts > 0}
                You have logged <strong class="num">{totalAttempts}</strong>
                attempt{totalAttempts === 1 ? "" : "s"}{streak > 1
        ...
        <table class="data-table domain-table">
            <thead>
                <tr>
                    <th
                        class="check-col"
                        title="Include in daily &amp; mixed sessions">Test</th
                    >
                    <th>Domain</th>
                    ...
                    <tr class:deselected={!selected.has(d)}>
                        <td class="check-col">
                            <input
                                ...
                                aria-label="Include {DOMAIN_LABELS[
                                    d
                                ]} in daily and mixed sessions"
                            />
                        </td>
                        ...
                        <td class="right">
                            <a
                                class="btn btn-sm"
                                href="/session?mode=domain&domain={d}&n=8"
                                >Train</a
                            >
                        </td>
                    </tr>
        ...
        <p class="rating-note muted">
            Ratings start at {BASE_RATING} and move like Elo: harder exercises penalize
            you less in case of a miss, but reward you better otherwise. The trend
            column shows your last 30 graded attempts. Daily and mixed sessions draw
            only from the
            <strong>{selectedCount}</strong>
            checked domain{selectedCount === 1 ? "" : "s"}; “Train” always runs
            the domain you pick. Your selection is carried along in exports.
        </p>
